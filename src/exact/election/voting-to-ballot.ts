...
import { Approval, Ranked, Score, Single } from "../../election/ballot";
...
export function cardinalVote<Voter, Candidate>(
    ...
    return (voter, candidates) =>
        new Map(Array.from(candidates, candidate => {
            const grade = min([
                nScores - 1,
                FRAC1.sub(disagree(voter, candidate)).mul(Math.floor(nScores)),
            ]);
...
export function balancedCardinalVote<Voter, Candidate>(
    ...
    return (voter, candidates) => {
        const prefs = new Map(Array.from(candidates, candidate => [candidate, FRAC1.sub(disagree(voter, candidate))]));
        ...
        return new Map(Array.from(candidates, candidate => {
            const grade = min([
                nScores - 1,
                ((prefs.get(candidate)!.sub(minPref)).mul(nScores).div(maxPref)).floor(),
            ]);
            return [candidate, Number(grade)];
        }));
    };
}
